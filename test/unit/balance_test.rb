...
class BalanceTest < ActiveSupport::TestCase
  ...
  test "should count history of operations when there is one income and two expense with two different dates" do
    @family = families(:family_of_user_with_wallet_2)
    assert_equal Balance.history(@family, 1, 10).length, 6
  end
