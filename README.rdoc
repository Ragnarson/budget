...
Application is hosted on http://shellycloud.com. To push changes to server you have to add remote:
  git remote add budget git@git.shellycloud.com:budget.git
and then type:
  git push budget master
...
* Create database and assign its owner
  postgres=# CREATE DATABASE budget WITH OWNER budget;
Your database should be already configured. You're ready now to run application.
